...
public class ItemData_Equipment : ItemData
{
    ...
    public void ApplyItemEffects(Transform _enemyPosition) {
        foreach (ItemEffect effect in itemEffects) {
            effect.ExecuteEffect(_enemyPosition);
        }
    }//armor : Apply when player is attacked by enemy  sword : Apply when player is attacking enemy
    ...
    public override string GetDescription() {
        sb.Length = 0;
        descriptionLength = 0;
        AddItemDescription(strength, "Strenth");
        AddItemDescription(agility, "Agility");
        AddItemDescription(intelligence, "Intellegence");
        AddItemDescription(vitality, "Vitality");

        AddItemDescription(damage, "Damage");
        AddItemDescription(critChance, "CriyChance");
    ...
    private void AddItemDescription(int _value ,string name) {
        if(_value != 0) {
            if(sb.Length > 0) {
                sb.AppendLine();
            }
            if(_value > 0) {
                sb.Append( "+ " + _value + " " + name);
            }
            descriptionLength++;
        }
    }
